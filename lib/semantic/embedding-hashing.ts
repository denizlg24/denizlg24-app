import { SEMANTIC_DIMENSION, SEMANTIC_HASHING_MODEL } from "./constants";
...
export function embedTextWithHashing(text: string) {
  const vector = Array.from({ length: SEMANTIC_DIMENSION }, () => 0);
  const tokens = tokenize(text);

  for (const token of tokens) {
    const hash = hashToken(token);
    const index = hash % SEMANTIC_DIMENSION;
    ...
    if (token.length > 5) {
      ...
      vector[stemHash % SEMANTIC_DIMENSION] += stemHash & 1 ? 0.5 : -0.5;
    }
  }
