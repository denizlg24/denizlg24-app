...
export default function AuthenticatorPage() {
  ...
  const fetchCodes = useCallback(async () => {
    if (!API) return;
    const result = await API.GET<{ codes: IAuthenticatorCode[] }>({
      endpoint: "authenticator/codes",
    });
    if (!("code" in result)) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const result = await API.GET<{ accounts: IAuthenticatorAccount[] }>({
    ...
    load();
  }, [API, initialLoading, fetchCodes]);

  useEffect(() => {
    if (initialLoading || !API) return;

    pollingRef.current = setInterval(fetchCodes, 1000);
    return () => {
      if (pollingRef.current) clearInterval(pollingRef.current);
    };
  }, [initialLoading, API, fetchCodes]);
  ...
  const handleDelete = async () => {
    ...
    const result = await API.DELETE<{ message: string }>({
      endpoint: `authenticator/${deleteTarget._id}`,
    });
    setDeleting(false);
    if ("code" in result) {
